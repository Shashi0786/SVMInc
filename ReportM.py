...
class NomeTest(BaseCase):
    def login_to_swag_labs(self):
        ...
        time.sleep(1)
        self.click('#company')
        self.click('#company > div > ul > li:nth-child(3) > a')
        self.wait_for_ready_state_complete
        self.click('#location')
    ...
    def Send_selenium_report(self, recipients, Body, directory, subject, filesName):
        ...
        server.starttls()
        server.ehlo()
        server.login('erpokhla@tetsnrai.com', 'erpokhla@123')  # if applicable
    ...
    def test1_Stock_Report(self):
        ...
        self.wait_for_ready_state_complete
        self.select_option_by_text('[name="locationID"]', 'ACC STORE-M', timeout=50)
        self.wait_for_ready_state_complete
        ...
        self.click('input[name="EndDate"]', timeout=50)
        self.date_Selection('div.bs-datepicker-body > table > tbody > tr > td > span[class="ng-star-inserted"]')
        self.click('[class="btn btn-info btn-sm"]')
        ...
        self.refresh_page()
        self.wait_for_ready_state_complete
        self.select_option_by_text('[name="locationID"]', 'FABRIC STORE-M', timeout=50)
        self.wait_for_ready_state_complete
        self.select_option_by_text( '[name="ledgerTypeID"]', 'Store Wise Summary', timeout=50)
        self.wait_for_ready_state_complete
        ...
        subject = 'Stock Report Manesar ' + datetime.now().strftime('%#d-%b-%Y')
        Body = '''<html>
            <head></head>
            <body>
    ...
    def test2_Production_Report(self):
        ...
        if datetime.today().strftime("%#A") == 'Monday':
            ...
            Ydate = datetime.today() - timedelta(days=1)
        # recipients = ['erpokhla@tetsnrai.com']
        recipients = ['upandey@tetsnrai.com', 'gmmanesar@tetsnrai.com', 'patomd@tetsnrai.com' ,'sandeepm@tetsnrai.com', 'prod.cord@tetsnrai.com', 'docs.msr@tetsnrai.com', 'ieimt@tetsnrai.com', 'packing@tetsnrai.com', 'accountsimt@tetsnrai.com', 'accstoreimt@tetsnrai.com',
                      'snarang@tetsnrai.com', 'raman@tetsnrai.com ', 'cutting@tetsnrai.com', 'erpokhla@tetsnrai.com', 'operationsmanesar@tetsnrai.com', 'finishmsr@tetsnrai.com', 'yuvraj@tetsnrai.com', 'vivek@tetsnrai.com', 'fsmanesar@tetsnrai.com']
        subject = 'Production Detail Report Manesar ' + Ydate.strftime('%#d-%b-%Y')
        Body = '''<html>
            <head></head>
            <body>
                <p style="margin: 0;">Dear All</p>
    ...
    def test3_Procurement_Report(self):
        ...
        header_format = workbook.add_format( {"valign": "vcenter", "align": "center", "bg_color": "#3C4748", "bold": True, 'font_color': 'white', 'font_size': 14})
        worksheet.autofit()
        for col_num, value in enumerate(df.columns.values):
            worksheet.write(0, col_num, value, header_format)
        ...
        directory = r'E:/ScriptsFiles/downloaded_files/'
        # recipients = ['erpokhla@tetsnrai.com']
        recipients = ['prod.cord@tetsnrai.com', 'upandey@tetsnrai.com', 'gmmanesar@tetsnrai.com', 'sandeepm@tetsnrai.com', 'fsmanesar@tetsnrai.com', 'ieimt@tetsnrai.com', ' packing@tetsnrai.com',
                      ' finishmsr@tetsnrai.com', ' snarang@tetsnrai.com', 'cutting@tetsnrai.com', ' accstoreimt@tetsnrai.com', ' operationsmanesar@tetsnrai.com', ' erpokhla@tetsnrai.com']
        subject = 'Procurement Status Manesar ' + datetime.now().strftime('%#d-%b-%Y')
        Body = '''<html>
            <head></head>
            <body>
                <p style="margin: 0;">Dear All</p>
